...
public interface ProyectoPersonaDao extends JpaRepository<ProyectoPersona, Long> {
    ...
    @Query(value = " SELECT ppe.PPE_PROYECTO , p.PRO_DESCRIPCION, ppe.PPE_PERSONA,  ppe.PPE_TOTAL_HS_REALES,p.PRO_PERSONA_LIDER, p.PRO_ABREVIATURA, p.PRO_EMPRESA_PRINCIPAL , p.PRO_ESTADO, tp.TIP_ACTIVO\n" +
"  FROM PROYECTOS_FASES_PERSONAS as ppe , ePM_PROYECTOS p INNER JOIN TIPOS_PROYECTOS as tp on p.PRO_TIPO_PROYECTO = tp.TIP_CODIGO  where PPE_PERSONA = ?1  AND p.PRO_CODIGO = ppe.PPE_PROYECTO and p.PRO_ESTADO = 37 AND tp.TIP_ACTIVO= 'S'  order by ppe.PPE_PROYECTO DESC ;",
            nativeQuery = true)
